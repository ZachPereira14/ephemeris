...
def plot_schedule_with_plotly(schedule_df):
    """
...
def optimize_schedule(df, magnitude_limit=(0, 14.5), air_mass_lim=True, setup_time=False, 
    ...
    for index, row in valid_candidates_df.iterrows():
        if last_end_time is None or row['Transit Start Time'] >= last_end_time:
            optimized_schedule.append(row)
            last_end_time = row['Transit End Time']
        else:
    ...
    if not optimized_schedule_df.empty:
        ...
        if setup_time:
            adjusted_schedule_start_time = first_target_start - timedelta(minutes=30)
            adjusted_schedule_end_time = last_target_end + timedelta(minutes=30)
        else:
            adjusted_schedule_start_time = first_target_start
            adjusted_schedule_end_time = last_target_end

        optimized_schedule_df['Schedule Start Time'] = adjusted_schedule_start_time
        optimized_schedule_df['Schedule End Time'] = adjusted_schedule_end_time

    # Debugging outputs
    num_valid_schedules = len(valid_candidates_df)
    max_transit_count = len(optimized_schedule_df)
    
    plot_schedule_with_plotly(optimized_schedule_df)

    print(f"\nTotal number of valid schedules: {num_valid_schedules}")
    print(f"Maximum number of transits that can fit in a night: {max_transit_count}")
    
    
    '''# Calculate how many schedules share the maximum transit duration or time range
    max_transit_count = len(optimized_schedule_df)
    num_max_transit_schedules = count_max_schedules(all_schedules, optimized_schedule_df)
    print(f"Number of schedules with the maximum number of transit events: {num_max_transit_schedules}")'''
